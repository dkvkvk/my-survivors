...
- **游戏名**：《不退》（NO RETREAT）
- **类型**：2D 割草幸存者（类吸血鬼幸存者）
- **世界观**：中国修仙世界（魔渊妖潮 · 青冥山守夜）（青霓虹 + 红木/和纸 + 金）
...
- **胜利条件**：活满 15 分钟 **或** 击破 3 只妖王

## 先读这几份文档（按顺序）

1. `HANDOVER.md` —— 环境、架构、约定、**踩过的坑**、验证流程（最重要）
2. `WEAPON_SYSTEM.md` —— 法宝/技能系统设计（**唯一权威**，改设计先改它）
...
**已完成**：
- 核心割草循环、更次刷怪、4 种怪 + 妖王（三段冲锋 AI）、藏宝匣
- 灵珠 + 升级三选一（**现在只出属性卡**，法宝卡已移出）
- 局外：灵石 → 坊市万宝楼 4 种永久强化
- 分块无限地图（瓦片集 + 无缝地面）
- 主菜单 / 暂停 / 失败结算 / **胜利结算**（新增）
- 主菜单**代码驱动动效**（背景视差漂移 + 樱花粒子 + 霓虹呼吸）
- **P6 法宝/技能系统**：
  - 蓝条（灵力）+ 4 技能槽（键位 **1/2/3/4**）+ 冷却
  - 4 个技能效果（万剑归宗/刃风暴/烈日爆发/雷神之怒）
  - 法宝数据表 `weapons.gd`（4 把，上限 4）
  - **乾坤袋（按 B）**：法宝格 + 技能选择 + 材料 + 升级按钮
  - **法宝掉落 + 按 F 拾取 + 替换面板**
  - 材料掉落（玄铁/雷魄）+ 神通残卷掉落
  - 斩妖数累积（法宝升级条件）
  - 技能图标/材料图标已接入
...
- ⚠️ 游戏会 **暂停**（升级/结算/乾坤袋），driver 必须设 `process_mode = PROCESS_MODE_ALWAYS`，否则停摆
...
先**实机试玩调手感**（掉落率、蓝耗、冷却、技能威力、升级所需斩妖数），
调顺后再考虑加内容（第 5/6 种法宝、多技能、技能融合）。
