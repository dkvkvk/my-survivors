...
- **每把法宝 2 门神通 + 2 门融合（共 14 门，P7）**：万剑归宗/御剑疾影、剑气纵横/剑环外放、
    大日焚天/焚地火域、九天神雷/蓄雷引弧、风卷残云/穿云巨梭、十方雷网/符阵合围；
...
- **成就系统（P7）**：13 项（累计斩妖/局数/通关/妖王/法宝数/品阶/灵石/身份…），
   结算时统一评估并显示「★ 解锁成就：…」；主菜单「成 就」页可查进度（判据 `achievements`）
- **多角色（P7）**：守山人（均衡）/ 符修（符阵起手·灵力厚·血脆）/ 剑修（剑环起手·出手快·走位慢），
    主菜单选身份并存档；配色为代码调色（独立行走表的提示词已备）
...
1. **卡片描述文字压住标题行**：Label 用 `set_anchors_preset(PRESET_FULL_RECT)` 后，
   ...
   正解：卡片内部的元素一律用**正值**（名字 top=10、描述 top=56）。
2. **头像比设定尺寸大一圈**：`TextureRect` 的最小尺寸 = 贴图尺寸（96x96），
   只设 `size = 50x50` 会被顶大 → 压到右边的名字/描述。正解：
   `expand_mode = TextureRect.EXPAND_IGNORE_SIZE`。
3. **设置面板的三个滑杆跑到面板外**：`_slider()` 的 y 是**相对面板中心**的位移，
   面板只有 520 高（±260），传 460/560/660 就把滑杆甩到面板下方 400px 处。
   正解：y 用 -70/30/130。

**教训**：这三处光看代码都「像是对的」，全是截图后一眼看出来的。
所以：改 UI 之后**必须自己截图看**（`_shots/shot_multi.gd` 在游戏内抓视口原图，
无窗口遮挡/DPI 干扰，配合 `read_image` 直接看），并给布局加机器断言
（`check_settings.gd` 断言卡片内元素不重叠/不越界、标题行不压卡片）。

### 坑 #18：老存档把设置存成整数百分比（显示成 5000%）
早期版本滑杆写的是 `int(v*100)`（50 = 50%），后来统一成 0~1 浮点，
但老存档读出来是 50 → 滑杆显示「5000%」且被 clamp 到上限（玩家会以为设置坏了）。
正解：`save.gd::_migrate_settings()`——读档时发现 > 1.5 就当旧格式 ÷100 并回写一次。
**教训**：改存档里数值的**单位/量纲**时，必须同时写迁移，否则老玩家数据全废。

### 坑 #19：占位文字与常量脱节（HUD 写着「妖王 0/3」）
胜利线从 3 改成 5 时漏改了 `survivors_game.tscn` 里的占位文字；
平时每帧刷新的 `_process` 会覆盖它，但**开局选法宝时会暂停游戏**——
`_process` 不跑，玩家看到的就是那句过期的占位文字。
正解：`game.gd::_ready()` 里按常量刷一遍占位，场景文字也同步改。

### 成就回填（P7）
老存档在成就功能上线前就达标的情况（如守夜早就超过 10 分钟）不会自动解锁，
会显示「600/600 但没★」。正解：打开成就页时调 `SaveGame.sync_achievements()` 补记。
...
### ⚠️ 难度标定记录（2026-09-24，MS_BOT 实测 17 局）
- **15 分钟线是死内容**：妖王 120/300/480s 刷出，斩 3 只即胜利 → 游戏 **8 分钟就结算**，
  \`SURVIVE_WIN_TIME=900\` 永远够不到。已把 \`VICTORY_BOSS_KILLS\` 改为 **5**
  （妖王 120/300/480/660/840s，第 5 只的胜利线与 15 分钟线收拢）。
- **等级爆炸**：\`xp_for_level = min(3+2L, 25)\` 在 **25 点封顶** → 后期每 25 点经验升一级，
  机器人 8 分钟升到 **300 级**（≈每秒弹一次三选一）。已改为超线性曲线
  （\`3 + 2L + 0.18·L^1.7\`）：480 秒约 55 级、900 秒约 75 级。
