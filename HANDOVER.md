...
- **开局选武器**（4 选 1，暂停游戏；手里剑是固定基础武器，选它 = 起手 Lv3）+ 掉落率 4%→12% + 开局保底
...
| `mob.gd` | 怪+首领（setup_boss 升格）/三段 AI/受击击退/掉落（宝石/金币/宝箱）/**专属能力**（分裂·飞扑·扑击·撞碎障碍·卡墙自愈） |
...
10b. **弹窗层级必须高于 HUD**：`HUD` 是 `layer = 10`，而 GameOver/LevelUpUI/PauseUI/VictoryUI 原来都是默认 `layer = 1`，
    于是结算界面上还显示击杀/存活/经验条/等级/金币/蓝条，技能栏直接压在"回到主菜单"按钮上；
    HUD 里的低血量红色脉冲也会盖在结算背景上（截图看起来整张偏红）。**已把四个弹窗改成 `layer = 30`，背包 `25`**，
...
10d. **撞碎瓦片时别用接触点直接取格**：物理接触点经常正好压在瓦片边界线上，local_to_map() 会把它解析到隔壁的空格，
    于是 get_cell_source_id() 返回 -1，撞墙逻辑被 continue 掉——现象是"滑动碰撞明明有，墙却永远碎不了"（重甲兵就这么哑了一轮）。
