...
### 2026-09-24 · 修复替换面板死锁 + 难度二调（保底放宽 / XP 再降）

- MS_BOT 复测发现真 bug：`_close_replace()` 不解除暂停也不隐藏面板，
  玩家法宝满 4 格捡第 5 把、点完「替换/放弃」后游戏**永远停在暂停里**
  （P6 时代该面板不可达，加到 6 种法宝后才暴露）。已修 + 判据断言
- 难度二调：开局法宝保底放宽（PITY_KILLS 12→8、PITY_MAX 2→3、PITY_TIME 90→150），
  机器人实测开局 30 秒 3 把、60 秒满 4 把；XP 系数 0.18→0.12（少卡片下 0.18 偏硬）
- 阻塞已解除：此前替换面板死锁会把长跑全部卡死在 30~210 秒，900 秒/5 妖王的最终测量需重跑
- 全量判定：@@python tools/qa/loop_judge.py@@ → **退出码 0**，14/14 PASS
...
### 2026-09-24 · 成就系统（P7）：13 项累计统计成就

- `achievements.gd`：13 项（累计斩妖 100/1000/10000、局数、通关 1/5 次、妖王 3、
  满配 4 件、法宝五阶、灵石 1000、修为 60、用过 2 个身份）
- 统计与解锁都进存档（`save.gd` 新增 stats / unlocked；`submit_run` 多收一个 extra 字典）
- 结算界面（胜/负两处）显示「★ 解锁成就：…」；主菜单新增「成 就」页（✓/✗ + 进度 1234/2000）
- 新增判据 ⑯ `achievements`：清档后跑三局，断言统计累加、达成即解锁、**不重复解锁**、
  成就页条目数；**反向测试通过**（把门槛改大 → FAIL）；测试会恢复玩家真实存档
- 全量判定：@@python tools/qa/loop_judge.py@@ → **退出码 0**，17/17 PASS
...
### 2026-09-24 · 主菜单「界面糅杂」修复 + 首次真·视觉验收

- 用户截图反馈「界面糅杂」：定位到 3 处锚点/尺寸误用（详见 HANDOVER 坑 #17）
  - 卡片描述用 `offset_top = -60`（FULL_RECT 下 = 卡片上方 60px）→ 压住标题和上一行
  - 头像 TextureRect 被贴图最小尺寸顶成 96x96 → 压到名字/描述
  - 设置面板滑杆 y 是相对面板中心的，传 460/560/660 → 跑到面板下方 400px
- 顺带修：HUD 占位「妖王 0/3」→ 0/5（胜利线早改成 5，占位漏改，坑 #19）
  ；老存档设置单位的迁移（5000% → 50%，坑 #18）；成就页行距 + 老存档回填（★ 正确显示）
- **能力升级**：现在可以真正「看图」了——新增 `_shots/shot_multi.gd` 在**游戏内抓视口原图**
  （无窗口遮挡/DPI 干扰），配合 read_image 直接视觉验收；本轮 4 张截图逐一确认
  （主菜单 / 设置 / 成就 / 战斗开局），并给卡片布局加了机器断言防复发
- 全量判定：@@python tools/qa/loop_judge.py@@ → **退出码 0**，17/17 PASS

状态：awaiting-human

### 2026-09-24 · 「人物无法切换」：查明是观感问题 + 换色拉开

- 用户反馈「人物无法切换」。用真实鼠标事件（带窗口）验证：**点击与存档切换都正常**
  （`点击前=jian_xiu 目标卡=shou_shan -> 点击后=shou_shan OK`）
- 真因：三个身份共用同一张主角表，只靠 modulate 换色且**色差太淡**，
  玩家看不出区别 → 以为没切换。已把换色拉开到一眼可辨（橙黄/冰蓝/原青蓝），
  截图对比确认三个身份清晰可分，并与选卡头像配色一致
- 顺带记录两条坑：headless 不派发模拟鼠标事件（点击类断言必须带窗口跑，
  判据里自动跳过）；换色区分角色时色差必须够大（HANDOVER 坑 #20/#21）
- 新增回归：check_settings 里加「真实点击切换角色」断言（带窗口跑时生效）
- 全量判定：@@python tools/qa/loop_judge.py@@ → **退出码 0**，17/17 PASS

状态：awaiting-human

### 2026-09-24 · 难度系统（寻常 / 凶险 / 修罗）+ HUD 身份可见

- 机器人实测「对真人偏易」，所以把难度交给玩家自己调：`balance.gd::DIFFICULTIES` 一处定义，
  影响**怪物血量 / 接触伤害 / 刷怪间隔 / 收益**四处（经验与灵石 ×1.3 / ×1.7）
- 存档复用 settings 字典（不改结构），非法值回退寻常；菜单在身份标题左侧加三个难度钮
- HUD 左下与胜/负结算界面写「身份 · 难度」——三个身份只靠配色区分（用户选了方案 C），
  必须把名字写出来玩家才知道自己在玩谁
- 判据：check_settings 断言难度按钮/写存档/倍率生效/apply_difficulty 真放大血量与伤害/非法回退
- 顺带修：胜利文案「斩三妖王」→ 按 `VICTORY_BOSS_KILLS` 常量取（改胜利线时漏改的又一处）
- 踩坑记录：`%HUD` 找不到（CanvasLayer 没开唯一名）+ 新标签压在灵力条上（坑 #22）
- 全量判定：@@python tools/qa/loop_judge.py@@ → **退出码 0**，17/17 PASS
