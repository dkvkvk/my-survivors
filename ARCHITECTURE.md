...
| `fire_field.tscn` + `fire_field.gd` | 焚地火域 / 焚天剑轮：留在地面持续灼烧的法阵（纯代码绘制，沿用法环画法） |
...
| `skills.gd` | ★ 神通表：14 门（6 法宝 ×2 + 2 融合）+ `FUSIONS` + `available_for()`（可选范围的唯一判定） |
...
| `audio.gd` 音量 | 设置菜单可调音乐/音效（`apply_volumes()`，存档在 settings 字典） |
