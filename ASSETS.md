...
> ⚠️ 接入后还要跑 `python tools/postprocess_mobs.py`（**提亮 1.18x + 提饱和 1.08x + 1px 暗描边**）：
> 修仙素材是夜色基调，不提亮的话阴风鸮（近黑）、蛮石傀（深绿）会和蓝灰地面糊在一起——描边与瓦片用同一套视觉语言。
...
> **字体是子集化的**（2026-09-22）：Fusion Pixel 全量 4.9MB，Web 首载太吃亏，所以按游戏实际用字裁到 420 字 / 46KB。
> 裁剪脚本 `tools/subset_font.py`（扫 `*.gd` / `*.tscn` / `*.tres` 收集字符 → pyftsubset → 逐码位校验）。
> **改了中文文案就跑一次**；忘了跑会被 L0 判据 `font-coverage` 拦住。
> 另外：原字体没有 `⚠`，妖王警告文案已从 `⚠ 妖王现世 ⚠` 改成字体自带的 `◆ 妖王现世 ◆`。
...
python tools/postprocess_mobs.py         # 妖物提亮 + 暗描边（可读性，接入后跑）
`@
